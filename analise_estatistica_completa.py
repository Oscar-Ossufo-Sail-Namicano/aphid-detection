...
Salve como: analise_prototipo_avancada.py
Uso:
...
ID, Condicao, Periodo, TP, FP, FN, TN
onde Condicao ∈ {"Antes","Pós"} e Periodo ∈ {"Manhã","Tarde"}
"""
...
# ---------- Config ----------
INPUT_CSV = "dados_afideos.csv"
OUT_DIR = "resultados_analise"
...
def compute_metrics(df):
    """Adiciona colunas Acuracia, Precisao, Recall, F1 tratando zeros e NaNs."""
    total = df[['TP','FP','FN','TN']].sum(axis=1)
    df['Acuracia'] = safe_div(df['TP'] + df['TN'], total)
    df['Precisao'] = safe_div(df['TP'], df['TP'] + df['FP'])
    df['Recall'] = safe_div(df['TP'], df['TP'] + df['FN'])
    df['F1'] = safe_div(2 * df['Precisao'] * df['Recall'], df['Precisao'] + df['Recall'])
    return df
...
def scheirer_ray_hare(data, factorA, factorB, dv):
    ...
    data: DataFrame
    factorA: nome coluna fator A (e.g., 'Condicao')
    factorB: nome coluna fator B (e.g., 'Periodo')
...
expected_cols = {'ID','Condicao','Periodo','TP','FP','FN','TN'}
...
group_stats = df.groupby(['Condicao','Periodo'])[metrics].agg(['mean','std','count'])
# Flatten columns
...
for met in metrics:
    # Modelo
    formula = f"{met} ~ C(Condicao) * C(Periodo)"
    # dropna for metric
    df_met = df[['Condicao','Periodo',met]].dropna()
    # Fit model
    try:
    ...
    if anova_table is None:
        ...
        try:
            ...
            check['normality_p'] = np.nan
        # Levene test across groups (Condicao x Periodo groups)
        groups = []
        for (a,b), sub in df_met.groupby(['Condicao','Periodo']):
...
for met in metrics:
    ...
    if not np.isnan(ck.get('levene_p', np.nan)) and ck['levene_p'] < 0.05:
        need_nonparam = True
    if need_nonparam:
        srh = scheirer_ray_hare(df, 'Condicao', 'Periodo', met)
        srh_results[met] = srh
    else:
...
for met in metrics:
    ...
    sns.barplot(x='Periodo', y=met, hue='Condicao', data=df, ci='sd', ax=ax)
    ...
    sns.boxplot(x='Periodo', y=met, hue='Condicao', data=df, ax=ax2)
...
# ---------- Compõe seção de resultados estatísticos ----------
...
for met in metrics:
    ...
    if srh_results[met] is not None:
        ...
        # Interpretação automática dos p-valores da ANOVA (se existente)
        if anova_table is not None:
            for factor in ['C(Condicao)', 'C(Periodo)', 'C(Condicao):C(Periodo)']:
...
print("Se quiser, posso adaptar o script para ler resultados brutos do seu detector (boxes/labels) e computar TP/FP/FN automaticamente.")